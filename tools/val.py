...
model_dir = './'
config_file = 'tools/sc_lane_config.py'
...
post_conf = -1.5 # Minimum confidence on the segmentation map for clustering
...
tmp_save_path = './dataset/tmp_sc_lane'
...
class PostProcessDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # GT is in road coordinate : camnormal2road @ cam2camnormal @ GT_in_cam
...
def val_multiple_epochs():
    ...
    model_paths = sorted([model_dir + file for file in model_paths if file.startswith("ckpt")])
    # distances = ((199 - np.arange(200)) / 2 + 3).reshape(-1, 1)
    for model_path in model_paths:
        print(model_path)
        model = configs.val_model()
